...
def assess_episode(
    ...
    *,
    ...
    for step in steps:
        ...
        if _is_valid_mask(step.get("action_mask") if isinstance(step, Mapping) else None):
            mask_ok += 1
        if isinstance(step, Mapping) and step.get("action_request") is not None:
            action_labels += 1
        ...
        try:
            ...
            target_name = ""
        if previous_target is not None and target_name and previous_target and target_name != previous_target:
            target_switches += 1
        if target_name:
    ...
    mask_integrity = mask_ok / denominator
    action_coverage = action_labels / denominator
    timing_integrity = max(0.0, 1.0 - (timestamp_violations + gap_count) / max(total - 1, 1))
    continuity = max(0.0, 1.0 - (gap_count + target_switches) / max(total - 1, 1))
...
def _segment_id(episode_id: str, run_index: int, start: int, end: int) -> str:
    raw = f"{episode_id}:{run_index}:{start}:{end}".encode("utf-8")
...
def segment_episode(
    ...
    for run_index, run in enumerate(_contiguous_runs(steps, max_gap_ms=config.max_gap_ms)):
            ...
            segments.append(
                {
                    ...
                    "segment_id": _segment_id(episode_id, run_index, start, end),
                    "episode_id": episode_id,
                    "run_index": run_index,
                    ...
                    "feature_vectors": [list(step["observation"]["feature_vector"]) for step in selected],
                    "action_masks": [step["action_mask"] for step in selected],
                    "action_requests": [step.get("action_request") for step in selected],
                    "executor_results": [step.get("executor_result", {}) for step in selected],
                    "rewards": [step.get("rewards", {}) for step in selected],
                    "timestamps_ms": [int(step["observation"]["timestamp_ms"]) for step in selected],
                    "step_indices": [int(step["observation"]["step_index"]) for step in selected],
                }
            )
    return segments
...
def split_name(episode_id: str, config: DatasetConfig | None = None) -> str:
    config = config or DatasetConfig()
    config.validate()
    digest = hashlib.sha256(f"{config.split_seed}:{episode_id}".encode("utf-8")).digest()
...
class _FeatureStats:
    def __init__(self, width: int = FEATURE_COUNT) -> None:
        self.width = width
        self.count = 0
        self.mean = [0.0] * width
        self.m2 = [0.0] * width
...
def build_dataset(
    ...
    *,
    ...
    for episode in episodes:
        ...
        episode_segments = segment_episode(episode, config)
        ...
        for segment in episode_segments:
            ...
            labeled_steps += sum(request is not None for request in segment["action_requests"])
    ...
    return {
        ...
        "statistics": {
            ...
            "episodes_accepted": accepted_episodes,
            ...
            "segments_by_split": {name: len(value) for name, value in splits.items()},
...
def write_dataset(dataset: Mapping[str, Any], output_dir: str | Path) -> dict[str, str]:
    root = Path(output_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths: dict[str, str] = {}

    manifest = {key: value for key, value in dataset.items() if key != "splits"}
    ...
    for split_name_value in ("train", "validation", "test"):
        ...
        with path.open("w", encoding="utf-8", newline="\n") as handle:
            for segment in dataset["splits"][split_name_value]:
                handle.write(json.dumps(segment, separators=(",", ":"), sort_keys=True, allow_nan=False))
                handle.write("\n")
...
def inspect_segment(segment: Mapping[str, Any]) -> dict[str, Any]:
    requests = segment.get("action_requests", [])
    rewards = segment.get("rewards", [])
    return {
        "segment_id": segment["segment_id"],
        "episode_id": segment["episode_id"],
        "length": segment["length"],
        "time_span_ms": int(segment["end_timestamp_ms"]) - int(segment["start_timestamp_ms"]),
        "labeled_steps": sum(request is not None for request in requests),
        "damage_dealt": sum(float(item.get("damage_dealt", 0)) for item in rewards),
        "damage_received": sum(float(item.get("damage_received", 0)) for item in rewards),
